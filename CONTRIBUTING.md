...
#### Empty/blank lines
All files (including third-party libraries) should end with exactly one empty line.
...
*[This coding style is loosely based on
...
Additionally:
* `modelfields.py` should be the name of files containing custom *model* fields.
* `formfields.py` should be the name of files containing custom *form* fields.
...
<br/>*If it's difficult to summarize what the migration does with a few words,
...
* where:
  ...
  * `<Noun or verb>` is a word concisely outlining the contents of the view.
    * If the view inherits from one of Django's top-level generic views (`ListView`, `DetailView`, `CreateView`, `UpdateView` or `DeleteView`),
      the word should be the same as the first part of the generic view's name.

#### View class order
Sort views in the same order as they appear in the app's `urls.py` (see [Path order](#path-order)).

#### View field order
A view inheriting from one of Django's generic views (the views that are part of the `django.views.generic` module) -
and possibly also `PermissionRequiredMixin` - should have its fields sorted in the following order:
...
* `ListView`: `"<objects>/"`;
* `DetailView`: `"<objects>/<pk>/"`;
* `CreateView`: `"<objects>/<pk>/add/"`;
* `UpdateView`: `"<objects>/<pk>/change/"`;
* `DeleteView`: `"<objects>/<pk>/delete/"`;
* where:
...
For example: `event/<Event:event>/occurrences/<int:pk>/add/`
...
urlpatterns = [
    ...
    path("events/<Event:event>/add/", ..., name='event_create'),
