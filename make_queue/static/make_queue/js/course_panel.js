...
function updateDisplay() {
    const startIndex = state.page * state.elementPerPage;
    const endIndex = (state.page + 1) * state.elementPerPage;
    ...
    for (let element of state.elements) {
        ...
        if (startIndex <= filteredElements && filteredElements < endIndex) {
            element.$element.toggleClass("display-none", false);
    ...
    const $paginationElement = $("#pagination-bar");
    $paginationElement.children().slice(2, -2).remove();
