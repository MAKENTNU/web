...
class EventListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        past = (
            queryset.past()
            .annotate(latest_occurrence=Max("timeplaces__start_time"))
            .order_by("-latest_occurrence")
            .prefetch_related(
                Prefetch(
                    "timeplaces",
                    queryset=(
                        TimePlace.objects.published().past().order_by("-start_time")
                    ),
        ...
        past_event_dicts = [
            ...
            for event in past
            if event.past_timeplaces
        ]

        return {
            **super().get_context_data(**kwargs),
            "future_event_dicts": sorted(
                future_event_dicts,
                key=lambda timeplace_: timeplace_["shown_occurrence"].start_time,
            ),
...
class AdminEventListView(PermissionRequiredMixin, ListView):
    ...
    def has_permission(self):
        ...
        return user.has_any_permissions_for(Event) or user.has_any_permissions_for(
            TimePlace
        )
    ...
    def get_queryset(self):
        return (
            Event.objects.annotate(
                ...
                num_future_occurrences=Count(
                    "timeplaces",
                    filter=Q(timeplaces__end_time__gt=timezone.localtime()),
                ),
            )
            .order_by("-latest_occurrence")
            .prefetch_related("timeplaces")
        )


class AdminEventParticipantsSearchView(
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.query_params:
            ...
            if search_string:
                found_users_with_tickets, found_users_without_tickets = (
                    self.get_users_matching_search(search_string)
                )
                context_data.update(
                    {
    ...
    @classmethod
    def get_users_matching_search(
        cls, search_string: str
        ...
        for search_fragment in search_string.split():
            ...
            for field in cls.user_search_fields:
                user_search_field_subquery |= Q(
                    **{f"{field}__icontains": search_fragment}
                )
            ...
            .prefetch_related(
                Prefetch(
                    ...
                    queryset=EventTicket.objects.annotate(
                        first_standalone_event_occurrence=Min(
                            "event__timeplaces__start_time"
                        ),
                        last_standalone_event_occurrence=Max(
                            "event__timeplaces__start_time"
                        ),
        ...
        for user in found_users:
            (
                found_users_with_tickets
                if user.tickets
                else found_users_without_tickets
            ).append(user)
...
class TimePlaceFormMixin(CustomFieldsetFormMixin, EventRelatedViewMixin, ABC):
    ...
    def get_form_kwargs(self):
        ...
        return insert_form_field_values(
            super().get_form_kwargs(), {"event": self.event}
        )
...
class TimePlaceUpdateView(
    PermissionRequiredMixin, TimePlaceRelatedViewMixin, TimePlaceFormMixin, UpdateView
):
...
class TimePlaceDuplicateCreateView(
...
class EventTicketCreateView(
    PermissionRequiredMixin, CustomFieldsetFormMixin, EventRelatedViewMixin, CreateView
):
    model = EventTicket
    ...
    def setup(self, request, *args, **kwargs):
        ...
        if time_place_pk is None:
            ...
            self.ticket_time_place = get_object_or_404(
                TimePlace, pk=time_place_pk, event=self.event
            )
            self.ticket_event = None

    def has_permission(self):
        can_register_for_time_place = (
            self.ticket_time_place
            and self.ticket_time_place.can_register(self.request.user)
        )
        can_register_for_event = self.ticket_event and self.ticket_event.can_register(
            self.request.user, fail_if_not_standalone=True
        )
    ...
    def get_form_kwargs(self):
        ...
        try:
            existing_ticket_for_user = EventTicket.objects.get(
    ...
    def form_valid(self, form):
        ...
        email_message_dict = {
            "type": "send_html",
            "html_render": email.render_html(
                self.request, {"ticket": ticket}, template_name="email/ticket.html"
            ),
            "text": email.render_text(
                self.request, {"ticket": ticket}, template_name="email/ticket.txt"
            ),
            # Pass a pure string, instead of the proxy object from `gettext_lazy`
            "subject": str(
                _("Your ticket for “{title}”!").format(
                    title=ticket.registered_event.title,
                )
            ),
...
class AdminEventTicketListView(
    PermissionRequiredMixin, EventRelatedViewMixin, ListView
):
    model = EventTicket
    ...
    def get_context_data(self, **kwargs):
        return super().get_context_data(
            **{
                ...
                "ticket_emails": ",".join(
                    ticket.email
                    for ticket in self.focused_object.tickets.filter(active=True)
                ),
                **kwargs,
            }
        )
...
class EventTicketCancelView(PermissionRequiredMixin, CleanNextParamMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        if self.ticket.timeplace:
            at_time_string = " " + _("at {time}").format(
                time=short_datetime_format(self.ticket.timeplace.start_time),
            )
        else:
            at_time_string = ""
        heading = _(
