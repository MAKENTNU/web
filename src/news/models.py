...
class NewsBase(models.Model):
    ...
    clickbait = MultiLingualTextField(
        verbose_name=_("clickbait"), widget=MultiLingualTextarea
    )
    image = CompressedImageField(
        upload_to=UploadToUtils.get_pk_prefixed_filename_func(
            news_subclass_directory_path
        ),
        max_length=200,
    ...
    image_description = MultiLingualTextField(
        ...
        help_text=_(
            "This should be a concise visual description of the image,"
    ...
    BASE_FIELDS_EXCLUDED_FROM_HISTORY = [
        "contain",
        "featured",
        "hidden",
        "private",
...
class Article(NewsBase):
    ...
    history = HistoricalRecords(
        excluded_fields=NewsBase.BASE_FIELDS_EXCLUDED_FROM_HISTORY
    )
...
class EventQuerySet(NewsBaseQuerySet):
    def future(self) -> "EventQuerySet[Event]":
        return (
            self.filter(timeplaces__end_time__gt=timezone.localtime())
...
class Event(NewsBase):
    ...
    number_of_tickets = models.IntegerField(
        default=0, verbose_name=_("number of available tickets")
    )
    ...
    history = HistoricalRecords(
        excluded_fields=[
            "number_of_tickets",
            *NewsBase.BASE_FIELDS_EXCLUDED_FROM_HISTORY,
        ]
    )
...
class TimePlaceQuerySet(models.QuerySet):
    def published(self) -> "TimePlaceQuerySet[TimePlace]":
        return self.filter(
            hidden=False, event__hidden=False, publication_time__lte=timezone.now()
        )

    def unpublished(self) -> "TimePlaceQuerySet[TimePlace]":
        return self.filter(
            Q(hidden=True)
            | Q(event__hidden=True)
            | Q(publication_time__gt=timezone.now())
        )
...
class EventTicket(models.Model):
    ...
    creation_date = models.DateTimeField(
        auto_now_add=True, verbose_name=_("original registration date")
    )
    active = models.BooleanField(default=True, verbose_name=_("active"))
    active_last_modified = models.DateTimeField(
        blank=True,
        default=timezone.localtime,
        verbose_name=_("last reactivated/canceled"),
    )
    ...
    class Meta:
        constraints = (
            models.CheckConstraint(
                check=Q(timeplace__isnull=False, event__isnull=True)
                | Q(timeplace__isnull=True, event__isnull=False),
                name="%(class)s_either_timeplace_or_event_is_set",
            ),
            models.UniqueConstraint(
                fields=("user", "timeplace"), name="%(class)s_unique_user_per_timeplace"
            ),
            models.UniqueConstraint(
                fields=("user", "event"), name="%(class)s_unique_user_per_event"
            ),
        )
        permissions = (
