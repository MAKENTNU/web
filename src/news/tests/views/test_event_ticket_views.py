...
class TestEventTicketViews(CleanUpTempFilesTestMixin, TestCase):
    ...
    def test__event_registration_view__can_only_be_viewed_with_correct_url_for_event_type(
        ...
        assert_response_status_code(
            reverse("event_ticket_create", args=[self.repeating_event.pk]),
            HTTPStatus.FORBIDDEN,
        )
        assert_response_status_code(
            reverse(
                "event_ticket_create",
                args=[self.repeating_event.pk, self.repeating_time_place.pk],
            ),
            HTTPStatus.OK,
        )
        assert_response_status_code(
        ...
        assert_response_status_code(
            reverse(
                "event_ticket_create",
                args=[self.standalone_event.pk, self.standalone_time_place.pk],
            ),
            HTTPStatus.FORBIDDEN,
        )
    ...
    def test__event_registration_view__can_only_be_viewed_with_correct_combination_of_event_and_time_place(
        ...
        repeating_time_place2 = duplicate(
            self.repeating_time_place, event=repeating_event2
        )
        ...
        assert_response_status_code(
            [repeating_event1.pk, repeating_time_place1.pk], HTTPStatus.OK
        )
        assert_response_status_code(
            [repeating_event1.pk, repeating_time_place2.pk], HTTPStatus.NOT_FOUND
        )
        assert_response_status_code(
            [repeating_event1.pk, self.standalone_time_place.pk], HTTPStatus.NOT_FOUND
        )
        assert_response_status_code(
            [repeating_event2.pk, repeating_time_place1.pk], HTTPStatus.NOT_FOUND
        )
        assert_response_status_code(
            [repeating_event2.pk, repeating_time_place2.pk], HTTPStatus.OK
        )
        assert_response_status_code(
            [repeating_event2.pk, self.standalone_time_place.pk], HTTPStatus.NOT_FOUND
        )
        assert_response_status_code(
            [self.standalone_event.pk, repeating_time_place1.pk], HTTPStatus.NOT_FOUND
        )
        assert_response_status_code(
            [self.standalone_event.pk, repeating_time_place2.pk], HTTPStatus.NOT_FOUND
        )
        assert_response_status_code(
            [self.standalone_event.pk, self.standalone_time_place.pk],
            HTTPStatus.FORBIDDEN,
        )
        ...
        for url in registration_urls:
            for data in ticket_data:
                with self.subTest(url=url, data=data):
                    ...
                    self.assertIn(
                        f"From: {settings.EVENT_TICKET_EMAIL}", printed_email_str
                    )
                    ...
                    self.assertEqual(
                        printed_email_str.count(str(ticket.uuid)),
                        6,
            ...
            with self.subTest(time_place_or_event=time_place_or_event):
                if isinstance(time_place_or_event, TimePlace):
                    registration_url = reverse(
                        "event_ticket_create",
                        args=[time_place_or_event.event.pk, time_place_or_event.pk],
                    )
                else:
                    registration_url = reverse(
                        "event_ticket_create", args=[time_place_or_event.pk]
                    )

                self.assertEqual(time_place_or_event.tickets.count(), 0)

                def assert_results_after_registering_for_event(
                    posted_data: dict,
                    *,
                    ...
                    if expected_form_instance:
                        ...
                        self.assertFalse(
                            EventTicket.objects.filter(pk=form_instance.pk).exists()
                        )
                ...
                created_ticket = assert_results_after_registering_for_event(
                    {"language": "nb"},
                    expected_form_instance=None,
                    expected_language="nb",
                    expected_comment="",
                )
                self.assertEqual(
                    created_ticket.active_last_modified, created_ticket.creation_date
                )
                ...
                reactivated_ticket = assert_results_after_registering_for_event(
                    ...
                    expected_form_instance=created_ticket,
                    expected_language="en",
                    expected_comment="A comment :)",
                )
                self.assertEqual(
                    reactivated_ticket.creation_date, created_ticket.creation_date
                )
                self.assertGreater(
                    reactivated_ticket.active_last_modified,
                    reactivated_ticket.creation_date,
                )

    def test__event_ticket_cancel_view__cancels_and_reactivates_tickets_as_expected(
        self,
    ):
        ticket_repeating = EventTicket.objects.create(
            user=self.user1, timeplace=self.repeating_time_place
        )
        ticket_standalone = EventTicket.objects.create(
            user=self.user1, event=self.standalone_event
        )
        ...
        for ticket in [ticket_repeating, ticket_standalone]:
            with self.subTest(ticket=ticket):
                ...
                assert_next_param_is_valid(
                    django_reverse("event_ticket_detail", args=[ticket.pk]), True
                )
                ...
                assert_next_param_is_valid(
                    reverse("event_detail", args=[ticket.registered_event.pk]), True
                )
                assert_next_param_is_valid(
                    django_reverse("event_detail", args=[ticket.registered_event.pk]),
                    True,
                )
