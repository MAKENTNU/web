...
class ArticleEventAndTimePlaceTests(TestCase):
    ...
    def test_str(self):
        ...
        time_place = self.create_time_place(
            event, relative_publication_time=0, relative_start_time=0
        )
    ...
    def test_event_queryset_correctly_filters_past_and_future_events(self, now_mock):
        ...
        TuplesTypeHint: TypeAlias = tuple[str, tuple[tuple[int, int], ...]]

        def create_event(
            event_type: Event.Type,
            title_and_relative_start_and_end_time_tuples: TuplesTypeHint,
        ) -> Event:
            title, relative_start_and_end_time_tuples = (
            ...
            for (
                relative_start_time,
                relative_end_time,
            ) in relative_start_and_end_time_tuples:
                TimePlace.objects.create(
                    event=event,
                    # Publication time should not affect the `past()` or `future()`
                    # methods
        ...
        def create_standalone(
            ...
            return create_event(
                Event.Type.STANDALONE, title_and_relative_start_and_end_time_tuples
            )
        ...
        def create_repeating(
            ...
            return create_event(
                Event.Type.REPEATING, title_and_relative_start_and_end_time_tuples
            )
    ...
    def test_timeplace_queryset(self):
        ...
        event_not_published = self.create_time_place(
            event, relative_publication_time=1, relative_start_time=1, hidden=False
        )
        event_future = self.create_time_place(
            event, relative_publication_time=-1, relative_start_time=1, hidden=False
        )
        event_past = self.create_time_place(
            event, relative_publication_time=-1, relative_start_time=-100, hidden=False
        )

        hidden_event_future_hidden = self.create_time_place(
            hidden_event, relative_publication_time=-1, relative_start_time=1
        )
        hidden_event_past_hidden = self.create_time_place(
            hidden_event, relative_publication_time=-1, relative_start_time=-1
        )
        hidden_event_future = self.create_time_place(
            hidden_event,
            relative_publication_time=-1,
            relative_start_time=1,
            hidden=False,
        )
        hidden_event_past = self.create_time_place(
            hidden_event,
            relative_publication_time=-1,
            relative_start_time=-1,
            hidden=False,
        ...
        self.assertSetEqual(
            set(TimePlace.objects.published()), {event_future, event_past}
        )
        self.assertSetEqual(
            set(TimePlace.objects.future()),
            {
                event_not_published,
                event_future,
                hidden_event_future_hidden,
                hidden_event_future,
            },
        )
        self.assertSetEqual(
            set(TimePlace.objects.past()),
            {
                event_past,
                hidden_event_past_hidden,
                hidden_event_past,
            },
        )
...
class EventTicketTests(TestCase):
    def setUp(self):
        ...
        self.standalone_event = Event.objects.create(
            title="Standalone event",
            event_type=Event.Type.STANDALONE,
            number_of_tickets=10,
        )
        self.repeating_event = Event.objects.create(
            title="Repeating event", event_type=Event.Type.REPEATING
        )
        self.standalone_time_place = TimePlace.objects.create(
            event=self.standalone_event, hidden=False
        )
        self.repeating_time_place = TimePlace.objects.create(
            event=self.repeating_event, hidden=False, number_of_tickets=10
        )

    def test_event_ticket_unique_constraints_are_enforced(self):
        self.assert_creating_ticket_raises_integrity_error(
            False, event=self.standalone_event, time_place=None
        )
        # Creating a second ticket with the same arguments should fail
        self.assert_creating_ticket_raises_integrity_error(
            True, event=self.standalone_event, time_place=None
        )

        self.assert_creating_ticket_raises_integrity_error(
            False, event=None, time_place=self.repeating_time_place
        )
        # Creating a second ticket with the same arguments should fail
        self.assert_creating_ticket_raises_integrity_error(
            True, event=None, time_place=self.repeating_time_place
        )

    def test_event_tickets_require_either_time_place_or_event_set(self):
        # Invalid combinations
        self.assert_creating_ticket_raises_integrity_error(
            True, event=None, time_place=None
        )
        self.assert_creating_ticket_raises_integrity_error(
            True, event=self.standalone_event, time_place=self.standalone_time_place
        )
        self.assert_creating_ticket_raises_integrity_error(
            True, event=self.repeating_event, time_place=self.repeating_time_place
        )
        self.assert_creating_ticket_raises_integrity_error(
            True, event=self.standalone_event, time_place=self.repeating_time_place
        )
    ...
    def assert_creating_ticket_raises_integrity_error(
        self, raises: bool, *, event: Event | None, time_place: TimePlace | None
    ):
        assertRaisesIntegrityError_withRollback(
            self,
            lambda: EventTicket.objects.create(
                user=self.user, event=event, timeplace=time_place
            ),
            raises=raises,
        )
    ...
    def test_event_tickets_have_equal__active_last_modified__and__creation_date__when_created(
        ...
        ticket = EventTicket.objects.create(
            user=self.user, timeplace=self.repeating_time_place
        )
        self.assertEqual(ticket.creation_date, now)
        self.assertEqual(ticket.active_last_modified, ticket.creation_date)
