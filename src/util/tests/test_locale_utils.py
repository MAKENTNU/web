...
class WeekUtilTest(SimpleTestCase):
    def test_exact_weekday_to_day_name_returns_expected_values(self):
        for week_offset in range(3):
            ...
            for day_index, day_name in ReservationRule.Day.choices:
                self.assertEqual(
                    exact_weekday_to_day_name(day_offset + day_index), day_name
                )
    ...
    def test_year_and_week_to_monday(self):
        self.assertEqual(
            year_and_week_to_monday(2017, 51), parse_datetime("2017-12-18 00:00")
        )

    def test_year_and_week_to_monday_start_of_year(self):
        self.assertEqual(
            year_and_week_to_monday(2019, 1), parse_datetime("2018-12-31 00:00")
        )
...
class TimedeltaTest(TestCase):
    def test_hour_conversion(self):
        ...
        self.assertEqual(
            28.25,
            timedelta_to_hours(timedelta(days=1, hours=3, minutes=60, seconds=900)),
        )
