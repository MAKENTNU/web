...
class APIMachineDataView(
    LoginRequiredMixin, MachineRelatedViewMixin, QueryParameterFormMixin, TemplateView
):
    ...
    def get_context_data(self, **kwargs):
        ...
        return {
            "reservations": [
                ...
                for r in self.machine.reservations.filter(
                    end_time__gte=timezone.now()
                ).exclude(pk=exclude_reservation_pk)
            ],
            "can_ignore_rules": any(
                quota.can_create_more_reservations(self.request.user)
                for quota in Quota.get_user_quotas(
                    self.request.user, self.machine.machine_type
                ).filter(ignore_rules=True)
            ),
            "rules": [
                {
                    "periods": rule.get_exact_start_and_end_times_list(
                        iso=False, wrap_using_modulo=True
                    ),
...
class APIReservationListView(
    MachineRelatedViewMixin, QueryParameterFormMixin, ListView
):
    model = Reservation
    ...
    def get_queryset(self):
        ...
        return (
            self.machine.reservations.filter(
                start_time__lt=end_time, end_time__gt=start_time
            )
            .select_related("user")
            .prefetch_related("event__event")
        )
    ...
    @staticmethod
    def build_reservation_dict(
        reservation: Reservation, request_user: User
    ) -> dict[str, str]:
        reservation_data = {
        ...
        if reservation.event:
            ...
            reservation_data["displayText"] = reservation.special_text
...
class APIReservationMarkFinishedView(
    ...
    def has_permission(self):
        ...
        return (
            user.has_perm("make_queue.change_reservation")
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not can_mark_reservation_finished(reservation):
            ...
            if reservation.start_time > now:
                message = _(
                    "Cannot mark reservation as finished when it has not started yet."
                )
            elif reservation.end_time <= now:
                message = _(
                    "Cannot mark reservation as finished when it has already ended."
                )
            else:
                message = None
            return UTF8JsonResponse(
                {"message": message} if message else {}, status=HTTPStatus.BAD_REQUEST
            )
...
class APIReservationDeleteView(
    ...
    def has_permission(self):
        user = self.request.user
        return (
            user.has_perm("make_queue.delete_reservation")
            or user == self.get_object().user
        )
