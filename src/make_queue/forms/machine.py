...
class MachineFormBase(forms.ModelForm):
    ...
    info_message_date = Machine._meta.get_field("info_message_date").formfield(
        disabled=True
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["status"] = forms.ChoiceField(
            choices=[(c, Machine.STATUS_CHOICES_DICT[c]) for c in status_choices],
...
class MachineDetailQueryForm(forms.Form):
    calendar_year = forms.IntegerField(
        required=False, min_value=datetime.MINYEAR, max_value=datetime.MAXYEAR
    )
                    ...
                    raise forms.ValidationError(
                        {
                            "calendar_week": forms.ValidationError(
                                message, code="invalid_calendar_week"
                            ),
                        }
                    )
