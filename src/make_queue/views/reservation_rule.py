...
class ReservationRuleListView(MachineTypeRelatedViewMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated:
            context_data["quotas"] = Quota.get_user_quotas(user, self.machine_type)
            if user.has_any_permissions_for(ReservationRule):
                context_data["rule_set_has_gaps"] = ReservationRule.rule_set_has_gaps(
                    self.machine_type
                )
        return context_data


class ReservationRuleFormMixin(
    MachineTypeRelatedViewMixin, CustomFieldsetFormMixin, ModelFormMixin, ABC
):
    model = ReservationRule
    ...
    custom_fieldsets = [
        {
            "fields": ("start_time", "days_changed", "end_time"),
            "layout_class": "ui three fields",
        },
        {
            "fields": ("start_days", "max_hours", "max_inside_border_crossed"),
            "layout_class": "ui three fields",
        },
    ]
    ...
    def get_form_kwargs(self):
        ...
        return insert_form_field_values(
            super().get_form_kwargs(), {"machine_type": self.machine_type.pk}
        )
    ...
    def get_back_button_text(self):
        return _("Reservation rules for {machine_type}").format(
            machine_type=self.machine_type,
        )
...
class ReservationRuleCreateView(
    PermissionRequiredMixin, ReservationRuleFormMixin, CreateView
):
...
class ReservationRuleUpdateView(
    PermissionRequiredMixin, ReservationRuleFormMixin, UpdateView
):
    permission_required = ("make_queue.change_reservationrule",)
    pk_url_kwarg = "reservation_rule_pk"

    def get_form_title(self):
        return _("Change Rule for {machine_type}").format(
            machine_type=self.machine_type,
        )


class ReservationRuleDeleteView(
