...
class MachineType(models.Model):
    ...
    usage_requirement = models.ForeignKey(
        CoursePermission,
        blank=True,
        null=True,
    ...
    def can_user_use(self, user: User):
        match self.usage_requirement.short_name:
            case CoursePermission.DefaultPerms.IS_AUTHENTICATED:
                return user.is_authenticated
            case CoursePermission.DefaultPerms.TAKEN_3D_PRINTER_COURSE:
    ...
    def can_use_3d_printer(user: User | AnonymousUser):
        ...
        if Printer3DCourse.objects.filter(username=user.username).exists():
            ...
            return True
        # This will typically only be the case for superusers
        return user.has_perm("make_queue.add_reservation")

    @staticmethod
    def can_use_special_printer(
        user: User | AnonymousUser, permission: CoursePermission
    ):
        if not user.is_authenticated:
            return False
...
class MachineQuerySet(models.QuerySet):
    def visible_to(self, user: User) -> "MachineQuerySet[Machine]":
        ...
        if (
            ...
            exclude_query |= Q(
                machine_type__usage_requirement=CoursePermission.objects.get(
                    short_name=CoursePermission.DefaultPerms.SLA_PRINTER_COURSE
                )
            )
...
class Machine(models.Model):
    ...
    info_message_date = models.DateTimeField(
        ...
        verbose_name=_("time the info message was changed"),
    )
    priority = models.IntegerField(
        null=True,
        blank=True,
    ...
    notes = models.TextField(
        ...
        help_text=_("This is only for internal use and is not displayed anywhere."),
    )
    ...
    def get_next_reservation(self):
        return (
            self.reservations.filter(start_time__gt=timezone.now())
            .order_by("start_time")
            .first()
        )
    ...
    def get_status(self):
        ...
        if self.reservations_in_period(
            timezone.now(), timezone.now() + timedelta(seconds=1)
        ):
...
class MachineUsageRule(models.Model):
    ...
    def __str__(self):
        return _("Usage rules for {machine_type}").format(
