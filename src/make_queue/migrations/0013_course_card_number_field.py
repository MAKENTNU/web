...
def reverse_update_user_card_from_course(apps, schema_editor):
    ...
    for course in Course.objects.using(db_alias).filter(
        user__isnull=False, user__card_number__isnull=False
    ):
    ...
    operations = [
        migrations.AlterField(  # Rename database column for use when converting
            ...
            field=models.IntegerField(
                null=True, verbose_name="card number (EM)", db_column="old_card_number"
            ),
        ),
            ...
            field=card.modelfields.CardNumberField(
                ...
                validators=[
                    django.core.validators.RegexValidator(
                        "^\\d{10}$", "Card number must be ten digits long."
                    )
                ],
        ...
        migrations.RemoveField(  # Remove old field when converting is done
            model_name="printer3dcourse", name="old_card_number"
        ),
        # Move card number to user
        migrations.RunPython(
            update_user_card_from_course, reverse_update_user_card_from_course
        ),
