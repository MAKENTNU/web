...
class TestAdminUserQuotaListView(TestCase):
    def test_get_user_quota(self):
        ...
        Quota.objects.create(
            all=True, user=user, machine_type=machine_type, number_of_reservations=2
        )
        quota2 = Quota.objects.create(
            user=user, machine_type=machine_type, number_of_reservations=2
        )
        Quota.objects.create(
            user=user2, machine_type=machine_type, number_of_reservations=2
        )

        self.client.force_login(user)
        context_data = self.client.get(
            reverse("admin_user_quota_list", args=[user.pk])
        ).context
...
class TestAdminQuotaPanelView(TestCase):
    def setUp(self):
        ...
        self.quota1 = Quota.objects.create(
            all=True, machine_type=self.sewing_machine_type, number_of_reservations=2
        )
        self.quota2 = Quota.objects.create(
            all=True, machine_type=self.printer_machine_type, number_of_reservations=4
        )
    ...
    def test_admin_quota_panel_responds_with_expected_errors(self):
        ...
        assert_error_response_contains(
            "user=404",
            expected_json_dict={"field_errors": {"user": [user_404_not_found]}},
        )
        assert_error_response_contains(
            "asdf=asdf",
            expected_json_dict={
                "field_errors": {"user": [field_required]},
                **undefined_asdf_field,
            },
        )
        assert_error_response_contains(
