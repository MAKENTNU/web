...
class APIReservationListViewTests(TestCase):
    def setUp(self):
        ...
        self.admin.add_perms(
            "make_queue.can_create_event_reservation",
            "make_queue.can_view_reservation_user",
        )
        ...
        time_place = TimePlace.objects.create(
            event=event, start_time=now, end_time=now + timedelta(hours=2)
        )
    ...
    def test_responds_with_expected_json_for_various_datetimes(self):
        ...
        def assert_response_contains(
            *,
            ...
            expected_reservations: list[Reservation],
        ):
            for user, client in (
                ...
                with self.subTest(user=user):
                    ...
                    self.assertDictEqual(
                        ...
                        {
                            "reservations": [
                                APIReservationListView.build_reservation_dict(
                                    reservation, user
                                )
                    ...
                    for reservation, reservation_json in zip(
        ...
        assert_response_contains(
            ...
            expected_reservations=[self.reservation1],
        )
        # Seconds should be accepted
        assert_response_contains(
            start="2020-01-01 01:10:10",
            end="2020-01-01 01:50:50",
            expected_reservations=[self.reservation1],
        )
        # Time zones should be accepted
        assert_response_contains(
            start="2020-01-01T04:00+01:00",
            end="2020-01-01T03:00-01:00",
            expected_reservations=[self.reservation4],
        )

        # The inclusion precision of reservations should be on a level of seconds
        one_sec = timedelta(seconds=1)
        assert_response_contains(
            start=self.reservation1.end_time,
            end=self.reservation4.start_time,
            expected_reservations=[self.reservation2, self.reservation3],
        )
        assert_response_contains(
            start=self.reservation1.end_time - one_sec,
            end=self.reservation4.start_time + one_sec,
            expected_reservations=[
                self.reservation1,
                self.reservation2,
                self.reservation3,
        ...
        assert_response_contains(
            start=self.reservation2.start_time,
            end=self.reservation2.start_time + one_sec,
            expected_reservations=[self.reservation2],
        )
    ...
    def test_responds_with_expected_query_parameter_errors(self):
        def assert_error_response_contains(query: str, *, expected_json_dict: dict):
            for user, client in (
                ...
                with self.subTest(user=user):
                    response = client.get(
                        f"{self.machine1_url}?{query}" if query else self.machine1_url
                    )
        ...
        assert_error_response_contains(
            ...
            expected_json_dict={
                "field_errors": {
                    "start_date": [field_required],
                    "end_date": [field_required],
                }
            },
        )
        assert_error_response_contains(
            "start_date=2020-01-01",
            expected_json_dict={"field_errors": {"end_date": [field_required]}},
        )
        assert_error_response_contains(
            "end_date=2020-01-31",
            expected_json_dict={"field_errors": {"start_date": [field_required]}},
        )
        assert_error_response_contains(
            "start_date=2020-01-01&end_date=2020-02",
            expected_json_dict={"field_errors": {"end_date": [value_not_datetime]}},
        )
        assert_error_response_contains(
            "start_date=2020-01&end_date=2020-02-01",
            expected_json_dict={"field_errors": {"start_date": [value_not_datetime]}},
        )
        assert_error_response_contains(
            "start_date=asdf&end_date=asdf",
            expected_json_dict={
                "field_errors": {
                    "start_date": [value_not_datetime],
                    "end_date": [value_not_datetime],
                }
            },
        )

        assert_error_response_contains(
            "start_date=2020-01-01 01:50&end_date=2020-01-01 01:10",
            expected_json_dict={
                "field_errors": {
                    "start_date": ["This must be before 'end_date'."],
                    "end_date": ["This must be after 'start_date'."],
                }
            },
        )

        assert_error_response_contains(
            "start_date=2020-01-01&end_date=2020-01-31&asdf=asdf",
            expected_json_dict={**undefined_asdf_field},
        )
        assert_error_response_contains(
            "start_date=2020-01-01&asdf=asdf",
            expected_json_dict={
...
class TestAPIReservationMarkFinishedView(TestCase):
    ...
    def post_to(self, reservation: Reservation):
        return self.client.post(
            reverse("api_reservation_mark_finished", args=[reservation.pk])
        )

    def test_get_request_fails(self):
        response = self.client.get(
            reverse("api_reservation_mark_finished", args=[self.reservation1.pk])
        )
    ...
    def test_can_finish_existing_reservations_for_machine_out_of_order_or_on_maintenance(
        ...
        for machine_status in (Machine.Status.OUT_OF_ORDER, Machine.Status.MAINTENANCE):
            with self.subTest(machine_status=machine_status):
                ...
                now_mock.return_value = self.reservation1.end_time - timedelta(
                    minutes=5
                )
...
class TestAPIReservationDeleteView(TestCase):
    ...
    def test_delete_own_reservation_succeeds(self):
        response = self.client1.delete(
            reverse("api_reservation_delete", args=[self.reservation.pk])
        )
        self.assertEqual(response.status_code, HTTPStatus.OK)
    ...
    def test_can_cancel_existing_reservation_for_machine_out_of_order(self):
        self._test_can_cancel_existing_reservation_for_machine_with_status(
    ...
    def _test_can_cancel_existing_reservation_for_machine_with_status(
