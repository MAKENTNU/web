...
class ReservationCreateOrUpdateViewTestBase(TestCase, ABC):
    def setUp(self):
        ...
        self.machine = Machine.objects.create(
            machine_model="Test", machine_type=self.sewing_machine_type
        )
    ...
    def get_view(
        self,
        view_class: Type[ReservationCreateView] | Type[ReservationUpdateView],
        **kwargs,
    ):
        view = view_class()
        ...
        *,
        ...
        return ReservationForm(
            data=self.create_form_data(
                start_time_delta, end_time_delta, event, special, special_text
            )
        )
    ...
    def create_form_data(
        ...
        return {
            ...
            "event_pk": 0 if event is None else event.pk,
            "special": special,
...
class TestReservationCreateOrUpdateView(ReservationCreateOrUpdateViewTestBase):
    def test_get_error_message_non_event(self):
        form = self.create_form(
            start_time_delta=timedelta(hours=1), end_time_delta=timedelta(hours=2)
        )
    ...
    def test_get_error_message_event(self):
        form = self.create_form(
            start_time_delta=timedelta(hours=1),
            end_time_delta=timedelta(hours=2),
            event=self.event,
        )
        self.assertTrue(form.is_valid())
        reservation = Reservation(
    ...
    def test_get_error_message_too_far_in_the_future(self):
        form = self.create_form(
            start_time_delta=Reservation.FUTURE_LIMIT,
    ...
    def test_get_error_message_machine_out_of_order(self):
        ...
        self.assertEqual(
            view.get_error_message(form, reservation), "Maskinen er i ustand"
        )
    ...
    def test_get_context_data_non_reservation(self):
        ...
        context_data = view.get_context_data(
            machine_pk=self.machine.pk, start_time=start_time
        )
        context_data["machine_types"] = set(context_data["machine_types"])
    ...
    def test_post_valid_form(self):
        view = self.get_view(ReservationCreateView, pk=self.machine.pk)
        view.request = post_request_with_user(
            self.user,
            data=self.create_form_data(timedelta(hours=1), timedelta(hours=2)),
        )
        # Need to handle the valid form function
        valid_form_calls = {"calls": 0}

        def form_valid(_form, **_kwargs):
            valid_form_calls.update({"calls": valid_form_calls["calls"] + 1})
            return HttpResponse()

        view.form_valid = form_valid
...
class TestReservationCreateView(ReservationCreateOrUpdateViewTestBase):
    ...
    def test_only_users_with_3d_printer_course_can_view_create_reservation_page_for_3d_printers(
        ...
        self._test_only_internal_users_can_view_create_reservation_page_for_machine(
            self.user, machine
        )
    ...
    def _test_only_internal_users_can_view_create_reservation_page_for_machine(
        self, user: User, machine: Machine
    ):
        reservation_create_url = reverse("reservation_create", args=[machine.pk])
...
class TestReservationUpdateView(ReservationCreateOrUpdateViewTestBase):
    ...
    def test_form_valid_normal_reservation(self, now_mock):
        ...
        form = self.create_form(
            start_time_delta=timedelta(hours=1), end_time_delta=timedelta(hours=3)
        )
        self.assertTrue(form.is_valid())
        view = self.get_view(pk=reservation.pk)
        response = view.form_valid(form)
        ...
        self.assertEqual(response.status_code, HTTPStatus.FOUND)
        self.assertEqual(
            Reservation.objects.first().end_time,
    ...
    def test_form_valid_changed_machine(self, now_mock):
        ...
        self.machine = Machine.objects.create(
            name="M1", machine_model="Generic", machine_type=self.sewing_machine_type
        )
        form = self.create_form(
            start_time_delta=timedelta(hours=1), end_time_delta=timedelta(hours=3)
        )
...
class TestReservationListView(TestCase):
    def setUp(self):
        ...
        Printer3DCourse.objects.create(
            user=self.admin, username=self.admin.username, date=now
        )
        Printer3DCourse.objects.create(
            user=self.user, username=self.user.username, date=now
        )
        Quota.objects.create(
            all=True,
            machine_type=printer_machine_type,
        ...
        sewing_machine = Machine.objects.create(
            name="Bobbin hehe", machine_model="Janome", machine_type=sewing_machine_type
        )
    ...
    def assert_context_has(
        ...
        *,
        ...
        reservations_owned_by_MAKE: bool,
        has_admin_perms: bool,
    ):
        self.assertEqual(response.status_code, HTTPStatus.OK)
        context = response.context
        self.assertSetEqual(
            set_without_duplicates(self, context["reservations"]), reservations
        )
        self.assertEqual(
            context["reservations_owned_by_MAKE"], reservations_owned_by_MAKE
        )
    ...
    def test_responds_with_expected_context_data_for_all_owner_param_choices(self):
        ...
        self.assertEqual(
            self.user_client.get(self.MAKEs_reservations_url).status_code,
            HTTPStatus.FORBIDDEN,
        )

        self.assert_context_has(
            self.admin_client.get(self.MAKEs_reservations_url),
            reservations={
                self.special_reservation_by_admin,
                self.event_reservation_by_admin,
            },
            reservations_owned_by_MAKE=True,
            has_admin_perms=True,
        )

        # owner=ReservationListQueryForm.Owner.ME
        self.assert_context_has(
            self.user_client.get(self.my_reservations_url),
            reservations={
                self.normal_reservation_by_user,
                self.sewing_reservation_by_user,
            },
            reservations_owned_by_MAKE=False,
            has_admin_perms=False,
        )
        self.assert_context_has(
            self.admin_client.get(self.my_reservations_url),
            reservations={
                self.special_reservation_by_admin,
                self.normal_reservation_by_admin,
                self.event_reservation_by_admin,
    ...
    def test_responds_with_expected_query_parameter_errors(self):
        def assert_error_response_contains(
            client_: Client, *, query: str, expected_json_dict: dict
        ):
        ...
        qwer_invalid = (
            "Velg et gyldig valg. qwer er ikke et av de tilgjengelige valgene."
        )
        ...
        for user, client in (
            ...
            with self.subTest(user=user):
                assert_error_response_contains(
                    client,
                    query="",
                    expected_json_dict={"field_errors": {"owner": [field_required]}},
                )
                assert_error_response_contains(
                    client,
                    query="owner=qwer",
                    expected_json_dict={"field_errors": {"owner": [qwer_invalid]}},
                )
                assert_error_response_contains(
                    client,
                    query="asdf=asdf",
                ...
                assert_error_response_contains(
                    client,
                    query=f"owner={self.Owner.ME}&asdf=asdf",
