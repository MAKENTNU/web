...
def get_ldap_field(
    ldap_data: list[tuple[str, dict[str, list[bytes]]]], field: str
) -> str:
    """
...
def get_user_details_from_ldap(search_field: str, search_value: str) -> dict[str, str]:
    ...
    if ldap_data:
        return {
            field: get_ldap_field(ldap_data, field)
            for field in STANDARD_USER_DETAILS_FIELDS
        }
    return {}


def _get_user_details_from_user_field(
    field_name: str, field_value: str, use_cached: bool
) -> dict[str, str]:
    user_details = {}
    if use_cached:
