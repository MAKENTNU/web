...
class ViewTestCase(TestCase):
    ...
    def test_login_wrapper(self):
        ...
        fixed_num_queries = (
            # Number of queries that are always executed (currently only
            # `user.social_auth`)
            1
        )
        ...
        with self.assertNumQueries(0 + fixed_num_queries):
            self.assert_expected_values_after_login(
                user1,
                expected_username="user1",
                expected_full_name="New Name Nameson",
                expected_ldap_full_name="Name Nameson",
            )
        ...
        user2 = self.create_social_user(
            ...
            social_data_fullname="New LDAP Name",
        )
        ...
        with self.assertNumQueries(2 + fixed_num_queries):
            self.assert_expected_values_after_login(
                ...
                expected_full_name="New LDAP Name",
                expected_ldap_full_name="New LDAP Name",
            )
        ...
        self.assert_expected_values_after_login(
            user3,
            expected_username="ldap_username3",
            expected_full_name="Name Nameson",
            expected_ldap_full_name="Name Nameson",
        )
    ...
    def create_social_user(
        username,
        email_username,
        first_and_last_name: tuple[str, str],
        *,
        ldap_full_name,
        social_data_fullname,
    ):
        ...
        UserSocialAuth.objects.create(
            user=user, extra_data={"fullname": social_data_fullname}, uid=uuid.uuid4()
        )
        return user

    def assert_expected_values_after_login(
        self, user, *, expected_username, expected_full_name, expected_ldap_full_name
    ):
        request = HttpRequest()
        request.user = user
