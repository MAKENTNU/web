...
class IndexPageViewTests(CleanUpTempFilesTestMixin, TestCase):
    ...
    def create_event(cls, *, event_type: Event.Type, private=False) -> Event:
        ...
        return Event.objects.create(
            title=f"Event {num_existing_events + 1}",
    ...
    @classmethod
    def create_event_with_one_time_place(
        cls, *, event_type: Event.Type, start_time: datetime, private=False
    ) -> Event:
    ...
    def test_context_data_contains_expected_event_dict_values(self):
        def assert_context_event_dicts_equal(expected_event_dicts: list[dict]):
            ...
            self.assertListEqual(
                response_context["featured_event_dicts"], expected_event_dicts
            )
        ...
        self.create_time_place(
            start_time=now - timedelta(days=1), event=past_standalone
        )
        ...
        for event_num in range(1, IndexPageView.MAX_EVENTS_SHOWN + 1):
            with self.subTest(event_num=event_num):
                event = self.create_event_with_one_time_place(
                    ...
                    start_time=now + timedelta(days=event_num),
                )
    ...
    def test_context_data_contains_expected_private_events(self):
        ...
        public_event = self.create_event_with_one_time_place(
            event_type=Event.Type.STANDALONE,
            start_time=now + timedelta(days=1),
            private=False,
        )
        private_event = self.create_event_with_one_time_place(
            event_type=Event.Type.STANDALONE,
