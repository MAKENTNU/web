        ...
        future_events = (
            ...
            .prefetch_related(
                ...
                Prefetch(
                    "timeplaces",
                    queryset=(
                        TimePlace.objects.published().future().order_by("start_time")
                    ),
        ...
        future_event_dicts = [
            {
                ...
                if event.standalone
                else len(event.future_timeplaces),
            }
            for event in future_events
            if event.future_timeplaces
        ]

        sorted_future_event_dicts = sorted(
            future_event_dicts, key=lambda event: event["shown_occurrence"].start_time
        )
        articles = (
...
class AdminPanelView(PermissionRequiredMixin, TemplateView):
    ...
    def has_permission(self):
        ...
        from util.templatetags.permission_tags import can_view_admin_panel
