...
class MultiLingualFormField(forms.MultiValueField):
    ...
    def __init__(self, *args, max_length=None, languages=None, **kwargs):
        ...
        subfield_attrs = {
            ...
            **kwargs,
            # The `label` attribute is not used by the subfields, so override the one in
            # `kwargs`
            "label": None,
        }
        subfields = []
    ...
    def compress(self, data_list):
        ...
        structure = MultiLingualTextStructure(
            "", languages=self.languages, use_default_for_empty=True
        )
        if not data_list:
            return structure
...
class MultiLingualRichTextFormField(MultiLingualFormField):
    # Most of the manually added tags, attributes, styles and protocols, are based on
    # the HTML generated by CKEditor
    ALLOWED_TAGS = {
    ...
    # Unless we want to emulate the CKEditor code, it requires a different field class
    # for its subfields
    ...
    def should_bleach(self):
        ...
        if isinstance(self.widget, forms.MultiWidget):
            edit_source_config_name = settings.CKEDITOR_EDIT_SOURCE_CONFIG_NAME
            for subwidget in self.widget.widgets:
                if (
                    isinstance(subwidget, CKEditorWidget)
                    and subwidget.config_name == edit_source_config_name
                ):
                    return False
        return True
