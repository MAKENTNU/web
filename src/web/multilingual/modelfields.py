...
class MultiLingualTextField(models.TextField):
    ...
    def __init__(self, *args, widget=None, languages=None, **kwargs):
        # Allow for specification of a widget on creation, to allow for both textarea
        # and text input
        self.widget = widget or self.widget
        self.languages = languages or self.languages
        if isinstance(self.widget, type) and issubclass(
            self.widget, MultiLingualTextEdit
        ):
            self.widget = self.widget(languages=self.languages)
        self.use_default_if_empty = kwargs.pop(
            "use_default_if_empty", self.use_default_if_empty
        )
    ...
    def to_python(self, value):
        ...
        if isinstance(value, MultiLingualTextStructure):
            return value
        return MultiLingualTextStructure(
            value,
            languages=self.languages,
            use_default_for_empty=self.use_default_if_empty,
        )
    ...
    def get_prep_value(self, value):
        ...
        if isinstance(value, MultiLingualTextStructure):
            ...
            return json.dumps(
                value.languages,
                # Prevents replacing unicode characters with \u encoding, which would
                # have messed with searching
                ensure_ascii=False,
            )
        return value
