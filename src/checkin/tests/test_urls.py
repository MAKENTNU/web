...
class UrlTests(TestCase):
    def setUp(self):
        ...
        self.suggest_skill1 = SuggestSkill.objects.create(
            creator=self.profile1, title="Ferdighet 3", title_en="Skill 3"
        )
        self.suggest_skill2 = SuggestSkill.objects.create(
            creator=self.profile1, title="Ferdighet 4", title_en="Skill 4"
        )
        self.suggest_skill3 = SuggestSkill.objects.create(
            creator=self.profile2, title="Ferdighet 5", title_en="Skill 5"
        )
        self.suggest_skills = (
            self.suggest_skill1,
            self.suggest_skill2,
        ...
        self.user_skill1_1 = UserSkill.objects.create(
            profile=self.profile1,
            skill=self.skill1,
            skill_level=UserSkill.Level.BEGINNER,
        )
        self.user_skill1_2 = UserSkill.objects.create(
            profile=self.profile1, skill=self.skill2, skill_level=UserSkill.Level.EXPERT
        )
        self.user_skill2_2 = UserSkill.objects.create(
            profile=self.profile2,
            skill=self.skill2,
            skill_level=UserSkill.Level.EXPERIENCED,
        )
    ...
    def test_all_get_request_paths_succeed(self):
        path_predicates = [
            # urlpatterns
            # This path is currently not in use; see the comments `checkin/urls.py`
