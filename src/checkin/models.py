...
class Skill(models.Model):
    title = models.CharField(
        max_length=100, unique=True, verbose_name=_("title (Norwegian)")
    )
    ...
    image = CompressedImageField(
        upload_to=UploadToUtils.get_pk_prefixed_filename_func("skills"),
        blank=True,
        max_length=200,
        storage=OverwriteStorage(),
        verbose_name=_("illustration image"),
    )
...
class Profile(models.Model):
    ...
    image = models.ImageField(
        upload_to=UploadToUtils.get_pk_prefixed_filename_func("profiles"),
        blank=True,
        max_length=200,
        storage=OverwriteStorage(),
    ...
    last_checkin = models.DateTimeField(
        auto_now=True, verbose_name=_("last checked in")
    )
...
class SuggestSkill(models.Model):
    ...
    image = CompressedImageField(
        upload_to=UploadToUtils.get_pk_prefixed_filename_func("skills/suggestions"),
        blank=True,
        max_length=200,
        storage=OverwriteStorage(),
        verbose_name=_("illustration image"),
