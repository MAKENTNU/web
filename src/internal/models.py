...
class Member(models.Model):
    ...
    MAKE_email = models.EmailField(
        blank=True,
        validators=[WhitelistedEmailValidator(valid_domains=["makentnu.no"])],
    ...
    ntnu_starting_semester = SemesterField(
        null=True,
        blank=True,
        verbose_name=_("starting semester at NTNU"),
        help_text=_("Must be in the format [V/H][year], e.g. “V17” or “H2017”."),
    )
    date_joined = models.DateField(
        default=timezone.datetime.now, verbose_name=_("date joined")
    )
    date_quit_or_retired = models.DateField(
        null=True, blank=True, verbose_name=_("date quit or retired")
    )
    reason_quit = models.TextField(blank=True, verbose_name=_("reason quit"))
    comment = models.TextField(blank=True, verbose_name=_("comment"))
    active = models.BooleanField(default=True, verbose_name=_("is active"))
    guidance_exemption = models.BooleanField(
        default=False, verbose_name=_("guidance exemption")
    )
    ...
    minecraft_username = UnlimitedCharField(
        blank=True, verbose_name=_("Minecraft username")
    )
    last_modified = models.DateTimeField(auto_now=True, verbose_name=_("last modified"))

    history = HistoricalRecords(
        m2m_fields=[committees], excluded_fields=["last_modified"]
    )
    ...
    def phone_number_display(self):
        ...
        if (
            region_code_for_number(self.phone_number)
...
class SystemAccess(models.Model):
    ...
    name = models.fields.CharField(
        choices=NAME_CHOICES, max_length=32, verbose_name=_("system")
    )
    ...
    class Meta:
        constraints = (
            models.UniqueConstraint(
                fields=("name", "member"), name="%(class)s_unique_name_per_member"
            ),
        )
    ...
    def __str__(self):
        return _("Access for {member} to {name}: {has_access}").format(
            member=self.member, name=self.name, has_access=self.value
        )
    ...
    def visible_to(self, user: User) -> "SecretQuerySet[Secret]":
        all_existing_extra_view_perms = (
            Permission.objects.filter(
                secrets_with_extra_view_perm__extra_view_permissions__isnull=False
            )
            # Remove duplicates that can appear when filtering on values across tables
            .distinct()
            # The `content_type` field is used in `perm_to_str()` below
            .select_related("content_type")
        )
        extra_view_perms_user_is_missing = [
            perm_str
            for perm_str in all_existing_extra_view_perms
...
class Secret(models.Model):
    ...
    history = HistoricalRecords(
        m2m_fields=[extra_view_permissions],
        excluded_fields=["priority", "last_modified"],
    )
...
class Quote(models.Model):
    ...
    quoted = models.CharField(
        max_length=100,
        verbose_name=_("quoted"),
