...
class UrlTests(TestCase):
    def setUp(self):
        ...
        non_member_user = User.objects.create_user(
            username="NON_MEMBER", password=password
        )
        member_user = User.objects.create_user(username="MEMBER", password=password)
        member_editor_user = User.objects.create_user(
            username="MEMBER_EDITOR", password=password
        )

        internal_perms = (
            "internal.is_internal",
            "internal.view_member",
            "internal.view_secret",
        )
        member_user.add_perms(*internal_perms)
        member_editor_user.add_perms(
            *internal_perms,
            "internal.add_member",
            "internal.can_edit_group_membership",
            "internal.change_systemaccess",
        )
        ...
        self.all_clients = {
            self.anon_client,
            self.non_member_client,
            self.member_client,
            self.member_editor_client,
        }
        ...
        self.mentor_board_content_box = ContentBox.objects.create(
            url_name="mentor-board"
        )
        self.pr_board_content_box = ContentBox.objects.create(url_name="pr-board")
        self.MAKE_history_content_box = ContentBox.objects.create(
            url_name="make-history"
        )
        self.content_boxes = (
            self.home_content_box,
            self.dev_board_content_box,
            self.event_board_content_box,
            self.mentor_board_content_box,
            self.pr_board_content_box,
            self.MAKE_history_content_box,
        )

        self.secret1 = Secret.objects.create(
            title="Key storage box", content="Code: 1234"
        )
        self.secret2 = Secret.objects.create(
            title="YouTube account",
            content="<p>Email: make@gmail.com</p><p>Password: password</p>",
        )
    ...
    def _test_url_permissions(
        ...
        data: dict = None,
        *,
        allowed_clients: set[Client],
        expected_redirect_path: str = None,
    ):
    ...
    def test_permissions(self):
        ...
        self._test_internal_url(
            "GET", reverse_internal("member_detail", self.member.pk)
        )
        ...
        self._test_internal_url(
            "GET", reverse_internal("member_update", self.member.pk)
        )
        self._test_editor_url(
            "GET", reverse_internal("member_update", self.member_editor.pk)
        )
        ...
        path_data_assertion_tuples = (
            (
                "member_quit",
                {"date_quit_or_retired": "2000-01-01", "reason_quit": "Whatever."},
                lambda member: member.quit,
            ),
            (
                "member_status_update",
                {"status_action": MemberStatusForm.StatusAction.UNDO_QUIT},
                lambda member: not member.quit,
            ),
            (
                "member_retire",
                {"date_quit_or_retired": "2002-01-01"},
                lambda member: member.retired,
            ),
            (
                "member_status_update",
                {"status_action": MemberStatusForm.StatusAction.UNDO_RETIRE},
                lambda member: not member.retired,
            ),
        )
        for path, data, assertion in path_data_assertion_tuples:
            with self.subTest(path=path, data=data):
                self._test_editor_url(
                    "POST",
                    reverse_internal(path, self.member.pk),
                    data,
        ...
        for system_access in self.member.system_accesses.all():
            with self.subTest(system_access=system_access):
                ...
                self._test_url_permissions(
                    "POST",
                    reverse_internal(
                        "system_access_update", self.member.pk, system_access.pk
                    ),
                    {"value": True},
        ...
        for system_access in self.member_editor.system_accesses.all():
            with self.subTest(system_access=system_access):
                ...
                self._test_url_permissions(
                    "POST",
                    reverse_internal(
                        "system_access_update", self.member_editor.pk, system_access.pk
                    ),
                    {"value": True},
                    allowed_clients=allowed_clients,
        ...
        self._test_internal_url(
            "POST",
            reverse_internal("set_language"),
            {"language": "en"},
            expected_redirect_path="/en/",
        )
        self._test_internal_url(
            "POST",
            reverse_internal("set_language"),
    ...
    def test_all_get_request_paths_succeed(self):
        path_predicates = [
            ...
            *[
                Get(
                    reverse_internal("content_box_update", content_box.pk), public=False
                )
