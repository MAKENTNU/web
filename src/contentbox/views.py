...
class ContentBoxDetailView(DetailView):
    ...
    @classmethod
    def get_multi_path(
        cls, url_name: str, alt_url1: str, *other_alt_urls: str
    ) -> tuple:
...
class ContentBoxUpdateView(
    PermissionRequiredMixin, CustomFieldsetFormMixin, UpdateView
):
        ...
        try:
            ...
            if url_on_other_subdomain:
                ...
                return render(
                    ...
                    {
                        "form_title": _(
                            "Cannot change this ContentBox on this subdomain"
                        ),
                        "django_admin_change_url": reverse_admin(
                            "contentbox_contentbox_change", args=[content_box.pk]
                        ),
    ...
    def _get_page_title(self, prefixed_verb: str = None):
        ...
        html_text = (
            f"{prefix}<code>{self.object.url_name}</code> (<code>{url_path}</code>)"
        )
