...
@override_settings(
    ROOT_HOSTCONF=hosts.__name__,
    DEFAULT_HOST="test_main",
...
class SimpleModelAndViewTests(TestCase):
    ...
    def test_all_paths_of_multi_path_content_box_retrieve_correctly(self):
        ...
        for path in paths_to_test:
            with self.subTest(path=path):
                ...
                self.assertIn("contentbox", response.context)
                self.assertEqual(
                    response.context["contentbox"].url_name,
                    multi_path_content_box_url_name,
                )
    ...
    def test_edit_page_contains_correct_error_messages(self):
        ...
        def assert_response_contains_error_message(posted_content: str, error: bool):
            data = {
                ...
                for subwidget_name in MultiLingualTextEdit.get_subwidget_names("title")
            } | {
                subwidget_name: posted_content
            ...
            self.assertEqual(
                response.status_code, HTTPStatus.OK if error else HTTPStatus.FOUND
            )
            self.assertInHTML(
                "Manglende språk", response.content.decode(), count=1 if error else 0
            )
    ...
    def test_edit_view_has_expected_form_class(self):
        ...
        def assert_edit_page_response_with(
            *, status_code: int, form: Type[BaseForm] | None
        ):
            ...
            if form:
                ...
                self.assertTrue(
                    response.context is None or response.context.get("form") is None
                )
        ...
        self.content_box1.extra_change_permissions.add(
            get_perm(can_change_rich_text_source_perm)
        )
        # User is not allowed
        ...
        assert_edit_page_response_with(
            status_code=HTTPStatus.OK, form=EditSourceContentBoxForm
        )
...
class MultiSubdomainTests(TestCase):
    def setUp(self):
        ...
        self.internal_user__internal_client.get(
            reverse(INTERNAL_TEST_URL_NAME, host="test_internal")
        )
        self.public_content_box = ContentBox.objects.get(url_name=TEST_URL_NAME)
        self.internal_content_box = ContentBox.objects.get(
            url_name=INTERNAL_TEST_URL_NAME
        )

        self.public_edit_url = reverse(
            "content_box_update", args=[self.public_content_box.pk]
        )
        self.public_admin_edit_url = reverse_admin(
            "contentbox_contentbox_change", args=[self.public_content_box.pk]
        )
        self.internal_edit_url = reverse(
            "content_box_update",
            args=[self.internal_content_box.pk],
            host="test_internal",
        )
        self.internal_admin_edit_url = reverse_admin(
            "contentbox_contentbox_change", args=[self.internal_content_box.pk]
        )
    ...
    def test_content_box_edit_urls_are_only_accessible_with_required_permissions(self):
        ...
        self.assertEqual(
            self.internal_user__public_client.get(self.public_edit_url).status_code,
            HTTPStatus.OK,
        )
        self.assertEqual(
            self.internal_admin__public_client.get(self.public_edit_url).status_code,
            HTTPStatus.OK,
        )
        self.assertEqual(
            self.superuser__public_client.get(self.public_edit_url).status_code,
            HTTPStatus.OK,
        )

        def assert_staff_can_change_through_django_admin(
            can_change: bool, client: Client, url: str
        ):
        ...
        self.assertEqual(
            self.superuser__internal_client.get(self.internal_edit_url).status_code,
            HTTPStatus.OK,
        )
        # Only staff can change content boxes through Django admin
        assert_staff_can_change_through_django_admin(
            True, self.internal_admin__admin_client, self.public_admin_edit_url
        )
        assert_staff_can_change_through_django_admin(
            True, self.internal_admin__admin_client, self.internal_admin_edit_url
        )
        assert_staff_can_change_through_django_admin(
            True, self.superuser__admin_client, self.public_admin_edit_url
        )
        assert_staff_can_change_through_django_admin(
        ...
        self.assertRedirects(
            self.internal_user__admin_client.get(self.internal_admin_edit_url),
